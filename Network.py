...
class Network():
    ...
    def trainTillInterupt(self,trainingData, printTime = 10000):
        print("starting timed training printing every " + str(printTime) + " cycles")
        try:
            count = 0
            while True:
                # Run net on data
    ...
    def lambdaTrain(self, callback, trainingData, trainingTime = 5, printCount = 1):
        print("starting training for " + str(trainingTime) + " cycles")
        # Var to store the number to print on based on the count of prints we want
        printTime = trainingTime/printCount
        ...
        for count in range(trainingTime):
            # Run net on data
            self.runData(trainingData)

            # Check data output from -runData- and get the error
            self.calcError(callback(trainingData, self.Nodes[-1]))
    ...
    def lambdaTrainTillInterupt(self, callback, trainingData, printTime = 10000):
        print("starting timed training printing every " + str(printTime) + " cycles")
        try:
            count = 0
            while True:
                ...
                # Check data output from -runData- and get the error
                self.calcError(callback(trainingData[0], self.Nodes[-1]))
    ...
    def calcError(self, answer):
        ...
        for index in range(len(self.Error) - 1, -1, -1):
            ...
            if index == len(self.Error) - 1:
                self.Error[index] = answer - self.Nodes[-1]
...
if __name__ == "__main__":
    ...
    net = Network([dataSize,5,25,5,answerSize])
